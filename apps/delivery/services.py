...
from apps.accounts.permissions import require_same_branch
...
def create_delivery_order(*, sale, platform, external_order_no, user):
    ...
    if not require_same_branch(user, sale.branch_id):
    ...
    log_event(user=user, branch=sale.branch, action='CREATE', entity='DeliveryOrder', entity_id=order.pk,
...
def update_delivery_status(*, order, status, user, reason=''):
    ...
    if not require_same_branch(user, order.sale.branch_id):
    ...
    log_event(user=user, branch=order.sale.branch, action='STATUS_CHANGE', entity='DeliveryOrder', entity_id=order.pk,
...
def settle_delivery(*, order, amount, user, note=''):
    ...
    log_event(user=user, branch=order.sale.branch, action='SETTLE', entity='DeliverySettlement', entity_id=settlement.pk,
